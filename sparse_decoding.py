...
class SparseDecoder():
    ...
    def applyLasso(self):
        ...
        fit_partial = partial(self.fitEN, model=self.EN, cdbook=cb_fit, calculate_r2=True)
        weights = np.array(list(map(fit_partial, self.lasso_pixs.values, self.alphas)))
        self.lasso_r2 = weights[:, -1:]
        self.lasso_r2 = xr.DataArray(weights[:, -1:].T.squeeze(),
    ...
    def applyOLS(self, elbow_thrs=[0.1, 0.1]):
        ...
        r2_numpy = np.zeros(w_table.shape[1])
        for j in range(w_table.shape[1]): # iterate over every pixel 
            ...
            wj = self.myfit(self.ols_pixs[j].values, ols, selbc, calculate_r2=True) # the ols fit
            r2_numpy[j] = wj[-1]
            wj = wj[:-1]
        ...
        self.ols_r2 = xr.DataArray(r2_numpy,
                                   coords={'x':('pixels', self.ols_table.coords['x'].values),
                                          'y':('pixels', self.ols_table.coords['y'].values),
                                          'z':('pixels', self.ols_table.coords['z'].values)}, 
                                       dims=['pixels'])
    ...
    def make3Dr2Map(self, r2_table, size=None):
        if self.size is None:
            raise ValueError("Field size is not set and w4d cannot be constructed")
        else:
            n_x = self.size[0]
        ...
        # declare a zero image and populate it with the deconvolved weights
        r2_3d = xr.DataArray(np.zeros((n_z, n_y, n_x)), 
                                dims=['z', 'y', 'x'],
                                coords = {'x': range(n_x), 'y': range(n_y), 
                                          'z': range(n_z) }) 
        
        r2_3d[r2_table['z'], r2_table['y'], r2_table['x']] = r2_table
        return r2_3d
    ...
    def myfit(int_row, model, cdbook, calculate_r2=False):
        ...
        if calculate_r2:
            ...
            r2 = SparseDecoder.myR2score(model, cdbook, int_row)
    ...
    def fitEN(self, int_row, alpha, model, cdbook, calculate_r2=False):
        ...
        if calculate_r2:
            coefs = model.coef_
            # r2 = model.score(cdbook, int_row)
            r2 = self.myR2score(model, cdbook, int_row)
            return np.append(coefs, r2)
        return list(model.coef_) # don't know why, but doesn't work without the list

    @staticmethod
    def myR2score(model, X, y_true):
    ...
    def createSpotTable(self, w_table, 
                        ...
                        projectWeights = True, gaus_sigma=0, weight_thresh=0.01, calculate_r2 = False):
        ...
        r2_maps = None
        if calculate_r2: 
            lr2 = self.make3Dr2Map(self.lasso_r2)
            or2 = self.make3Dr2Map(self.ols_r2)
            if projectWeights and ('z') in lr2.dims: 
                lr2 = lr2.max('z')
                or2 = or2.max('z')
            r2_maps = {'lasso_r2' : lr2.values, 
                        'ols_r2' : or2.values
                      }
        if self.verbose:
        ...
        for bc in self.cb['target'].values:
            ...
            spots_g = SparseDecoder.segmentWeights(wmap.values, thresh_abs=thresh_abs, 
    ...
    @staticmethod
    def segmentWeights(w_img, thresh_abs=0.2, peak_footprint=2, peak_mindistnce=1, more_images=None):
        """ Given a weight image, perform segmentation and return the properties of the segmented regions.
            First peak_local_max is applied, followed by watershed.
            w_img: 2d or 3d numpy array
            thresh_abs: Absolute intensity threshold, a parameter to peak_local_max
            peak_footprint: the neighborhood shape to find local max. If an int, specifies the radius
                ...
                a numpy array specifying the neighborhood
            peak_mindistance: minimum distance between two peaks, refer to peak_local_max
            The mean intensity values will be calculated for additional images in `more_images` dictionary
        """
        wg = deepcopy(w_img.squeeze())
        ndims = len(wg.shape) # 2 or 3d    
        if type(peak_footprint) == int:
        ...
        basins = watershed(-wg, markers=label(peaks), mask=wg>0)
        properties = ('weighted_centroid', 'label', 'area', 'max_intensity',
                      'mean_intensity', 'equivalent_diameter', 'eccentricity', 
                      'area_bbox', 'area_convex', 'solidity', 'perimeter', 'feret_diameter_max')
        df = pd.DataFrame(regionprops_table(basins, wg, properties))
        if not more_images is None and type(more_images) is dict: 
            for key in more_images:
                df[key] = regionprops_table(basins, more_images[key], ['mean_intensity'])['mean_intensity']
    
        if df.shape[0] == 0:
            return df
        
        if ndims == 2:
            df = df.rename(columns={'weighted_centroid-0':'y',
                                    'weighted_centroid-1':'x'})
            df.insert(0, 'z', 0)
        else:
            df = df.rename(columns={'weighted_centroid-0':'z',
                                    'weighted_centroid-1':'y',
                                    'weighted_centroid-2':'x'})
            df = df.round({'z': 1})
        df = df.round({'x': 1, 'y': 1})
        df = df.rename(columns={'equivalent_diameter':'diameter',
                                'max_intensity':'weight_max',
