...
# spot_addr = "../3_Decoded/all_spots_filtered.tsv"
...
for fov in fovs: 
    ...
    if max_int == 'max':
        ...
        m_int = max_int
    this_blb = blob_log(normalize(anc_img, m_int, min_int), min_sigma=0.7, 
...
for g in genes:
    ...
    plotGene(spots_g, ax=ax, ptsize=4, alpha=0.6)
    plt.tight_layout()
    fig.savefig(os.path.join(destdir, "{}_rolonies.pdf".format(g)), dpi=250)
    plt.close()
...
_, vox, _ = getMetaData(metadataFile) # commented by Xuwen to use pixel size from param file
# vox = {'1':params['pixel_size_um'], '2':params['pixel_size_um']}
