...
if not params['skip_seg']:
    cellInfos = regionprops_table(mask, properties=['label', 'centroid', 'area'])
    centroid_df = pd.DataFrame(cellInfos)
    centroid_df.columns = ['cell_label', 'centroid_y', 'centroid_x', 'area']
