...
def removeOverlapRolonies(rolonyDf, FOVmap,FOV_areamap, x_col = 'x', y_col = 'y', removeRadius = 5.5): #, n_pools=5):
    ...
    removePerGene = functools.partial(removeOverlapRolonies_gene, rolonyDf=rolonyDf, FOVmap=FOVmap, FOV_areamap = FOV_areamap, x_col=x_col, y_col=y_col, removeRadius=removeRadius)
    reducedRolonies = []
...
def removeOverlapRolonies_gene(gene, rolonyDf, FOVmap, FOV_areamap, x_col = 'x', y_col = 'y', removeRadius = 5.5):
# def removeOverlapRolonies_gene(thisGene_rolonies, fov_map, x_col = 'x', y_col = 'y', removeRadius = 5.5):
    ...
    for pos in sorted(thisGene_rolonies['fov'].unique()):
        for overlapping_fov in FOVmap[pos]:
            if overlapping_fov in FOV_scaned:
                continue
            # getting the overlapping area
            x1, y1 = FOV_areamap[(pos, overlapping_fov)][0]
            x2, y2 = FOV_areamap[(pos, overlapping_fov)][1]
            # getting the rolonies in the overlapping area
            overlapping_fov_rolonies = thisGene_rolonies.loc[thisGene_rolonies['fov'] == overlapping_fov]
            rolonies_to_remove = overlapping_fov_rolonies.loc[(overlapping_fov_rolonies[x_col].between(x1,x2)) & (overlapping_fov_rolonies[y_col].between(y1,y2))].index
            thisGene_rolonies = thisGene_rolonies.drop(rolonies_to_remove)
            FOV_scaned.append(overlapping_fov)
    return(thisGene_rolonies)

def findFOVmap(coords_df, fov_size = 2048):
    ...
    for i, fov in enumerate(coords_df.index):
        ...
        overlap_fov = np.where(dists < fov_size)
        overlap_fov = coords_df.index[overlap_fov]
        overlap_fov_dict[fov] = overlap_fov
        for j, fov2 in enumerate(overlap_fov):
            x1, y1 = this_fov
            x2, y2 = coord_mat[coords_df.index.get_loc(fov2), ]
            overlap_area_dict[(fov, fov2)] = ((max([x1,x2]), min([x1+fov_size, x2+fov_size])),(max([y1,y2]), min([y1+fov_size, y2+fov_size])))
    return(overlap_fov_dict, overlap_area_dict)
...
def trainClassifier(n_estimators=100, max_depth=4, class_weight='balanced', probability_threshold='infer', **kwargs):
    ...
    # ###### adding Slc26a4 spots in FOV101 as empty to the classifier
    # badspots = pd.read_csv("../3_Decoded/FOV101_rawSpots.tsv", 
    #                        index_col=0, sep="\t").assign(fov="FOV101")
    # badspots.loc[badspots['gene'] == "Slc26a4", "gene"] = "Empty"
    # spots_train.append(badspots)
    # ######
    
    spots_train = pd.concat(spots_train, ignore_index=True)

    """ 2. Prepping the data for the training """
    X = spots_train[labels].to_numpy()
    X_mean, X_std = X.mean(axis=0), X.std(axis=0)
    X = (X - X_mean) / X_std
    y = (spots_train['gene'].str.startswith('Empty')).to_numpy()
    ...
    ax[2].set_title("Decision boundary on non-empty spots")
    ...
    ax[6].set_title("Decision boundary on empty spots")
    ...
    for c in C:
        ...
        er.append((temp['gene'].str.startswith('Empty')).sum() / temp.shape[0])
    plt.figure()
    plt.plot(C, er)
    ...
    ax = axes[0]
    pca1, pca2 = 0, 1
    ax.scatter(XT[~y, pca1], XT[~y, pca2], s=1, alpha=.2, c='blue', label='non-empty')
    ax.scatter(XT[y, pca1], XT[y, pca2], s=1, alpha=.5, c='red', label='empty')
    ax.set_xlabel(f"PC{pca1+1}")
    ax.set_ylabel(f"PC{pca2+1}")
    ax.set_title("real data")
    ax.legend()
    
    ax = axes[1]
    ax.scatter(XT[:, pca1], XT[:, pca2], s=1, alpha=.2, c=probs_train)
    ax.set_xlabel(f"PC{pca1+1}")
    ax.set_ylabel(f"PC{pca2+1}")    
    ax.set_title("Empty rates")
    
    ax = axes[2]
    inds = probs_train > prob_thresh
    ax.scatter(XT[~inds, pca1], XT[~inds, pca2], s=1, alpha=.2, c='blue', label='non-empty')
    ax.scatter(XT[inds, pca1], XT[inds, pca2], s=1, alpha=.5, c='red', label='empty')
    ax.set_xlabel(f"PC{pca1+1}")
    ax.set_ylabel(f"PC{pca2+1}")
    ax.set_title("predictions")
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, "on_pca.png"), dpi=250)

    
    return rf1, (X_mean, X_std), lims, prob_thresh
...
labels = ['weight_max', 'area', 'weight_mean', 'diameter', 'eccentricity', 'area_bbox', 'area_convex', 'solidity', 'perimeter', 'feret_diameter_max', 'lasso_r2', 'ols_r2']

decoding_dir = params['dc_out'] # the main directory for decoding 
fov_size = params['fov_size'] # the size of the FOV in pixels
...
FOVmap, FOV_areamap = findFOVmap(FOVcoords, fov_size) # dictionary to show the nearest tiles to each FOV
...
for i, file in enumerate(all_files):
    ...
    fov_X = spots_fov[labels].to_numpy()
...
spots_overlapFree = removeOverlapRolonies(passed_spots, FOVmap, FOV_areamap, x_col='xg', y_col = 'yg', removeRadius=overlapRemovalRadius)
