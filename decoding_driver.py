...
def dc_fov(name, indir, regex, rounds, chans, codebook, min_norm, alpha, ENargs, elbow_thrs, chanCoefs, min_peak, outdir, is3D, gaus_sigma, weight_thresh):
    ...
    fov_spots = dcObj.createSpotTable(dcObj.ols_table, thresh_abs=min_peak, gaus_sigma=gaus_sigma, weight_thresh=weight_thresh, calculate_r2=True)
...
# Parallel(n_jobs=dc_npool, prefer='processes')(delayed(dcpartial)(name) for name in fov_names[101:211])
t2 = time.time()
